...
kernelLen = 0.2;kernelShift = 0.05;
spikeCountLen = 0.02;
...
for ii=1:totalUnits
   ...
   Response{ii,1} = zeros(totalStims,1);
   for kk=1:totalStims
...
onScreenMovie = zeros(totalStims,DIM(1)*DIM(2)*kernelLenFull);
for kk=1:totalStims
  stimOffset = stimOffsets(kk);
...
clearvars -except onScreenMovie Response DIM kernelLenFull screenPix_to_effPix ...
    DistToScreen totalStims conv_factor kernelLen totalUnits;


% CREATE LAPLACIAN MATRIX
%L = zeros(DIM(1)*DIM(2),DIM(1)*DIM(2),'single');
onScreenMovie = [double(onScreenMovie);zeros(DIM(1)*DIM(2)*kernelLenFull,DIM(1)*DIM(2)*kernelLenFull)];

%2D operator = [0,-1,0;-1,4,-1;0,-1,0];
bigCount = totalStims+1;
for kk=1:kernelLenFull
    for jj=1:DIM(1)
        for ii=1:DIM(2)
            ...
            onScreenMovie(bigCount,:) = tempMat(:)';bigCount = bigCount+1;
        end
    end
end
...
for ii=1:totalUnits
    ...
    for lambda = 1:length(bigLambda)
        ...
        onScreenMovie(totalStims+1:end,:) = onScreenMovie(totalStims+1:end).*(bigLambda(lambda)/prevLambda);
        fhat = pinv(onScreenMovie([1:train,totalStims+1:end],:))*constraints;
            
        tempF(lambda,:) = fhat;
        RMS(lambda) = norm(r(train+1:end)-onScreenMovie(train+1:end,:)*fhat)./sqrt(DIM(1)*DIM(2)*kernelLenFull);
        prevLambda = bigLambda(lambda);
    end
    [~,bestMap] = min(RMS);
    F(ii,:,:) = tempF(bestMap,:,:);
end
